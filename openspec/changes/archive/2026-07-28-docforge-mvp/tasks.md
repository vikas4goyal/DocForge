...
- [x] 17.14 Manually verify on a physical Android device and a physical iPhone; confirm no web or desktop artefacts exist anywhere in the repo. The repo half is automated (`tool/check_platforms.dart`, green). Android was verified on a Pixel 9 emulator at API 36: all three integration suites pass, PDFs are published to `Documents/DocForge/` and readable by another UID, and the MediaStore ownership limit was found and recorded in `public-library-and-page-table-creation/design.md`. **iOS remains outstanding** — the owner is running it on hardware; see `design.md` §32.
