...
- [x] 2.2 Add Tier-1 Detail Cubit/use-case tests and Tier-2 Detail component tests for imported pages, lazy previews, protected/missing/corrupt failures, and dashboard/detail consistency.
...
- [x] 2.5 Add Tier-2 OCR/extracted-text component coverage using real Cubits/use cases and page/text repository fakes for stored, embedded, fallback, progress, empty, and error states.
- [x] 2.6 Update the `import`, `browse_and_view`, and `share` Tier-3 robots/flows to prove a text-heavy imported PDF has matching Dashboard/Detail previews and locally extractable text.
...
- [x] 3.10 Add Tier-2 Dashboard and folder component tests with real Cubits/use cases for two-column phone and adaptive iPad/tablet grids, split-view resize, accessibility fallback, long metadata, lazy thumbnails, rounded search, empty/loading/error/no-results, empty/partial/all selection, archive, Trash confirmation, progress, partial failure, retry, dark mode, and offline use.
- [x] 3.11 Add fixture-backed `@Preview()` coverage and iPhone/iPad/Android phone/tablet light/dark/large-text goldens for two-column/adaptive grids, long two-line names, document/folder metadata, rounded search idle/focused/query/no-results, selection without reflow, working, and partial failure.
- [x] 3.12 Update the `browse_and_view` and `organise` robots and Tier-3 flows to assert compact/wide grid behavior, search/clear/open through keys and semantics, stable multi-selection, bulk archive, and confirmed bulk Trash in Dashboard and an open folder.
...
- [x] 4.7 Add Tier-2 Detail component tests for created-folder discovery, Root/current destinations, empty/error/retry, reviewed duplication, navigation callback count, long titles, large text, imported previews, and offline behavior.
- [x] 4.8 Add fixture-backed `@Preview()` entries and phone/tablet light/dark/large-text goldens for Detail default/loading/empty/error/long-title, move picker, duplicate review/progress/success/failure.
- [x] 4.9 Update the `organise` robot and Tier-3 flow to create a folder, load it from Detail, move a document, duplicate with a reviewed name/destination, and observe exactly one new Detail result.
...
- [x] 5.6 Add fixture-backed `@Preview()` entries and Viewer goldens for default/loading/error/long-title, jump dialog/validation, phone/tablet, light/dark, and large text.
...
- [x] 6.4 Add comprehensive `bloc_test` state-sequence coverage for cancel, progress, success, failure, retry, repeated-confirm suppression, and exactly-one navigation for every operation family.
...
- [x] 6.8 Add Tier-2 Editor component tests with real Cubit/use cases for every input/review/progress/result/failure path, two-output Split result, one-output navigation, in-place refresh, contextual actions, long titles, and large text.
- [x] 6.9 Add fixture-backed `@Preview()` entries and phone/tablet light/dark/large-text goldens for Editor and every operation's input, review, working, success, failure, empty, and long-content states.
- [x] 6.10 Update the `edit` robot and Tier-3 flow to perform split, merge, compress, watermark, protect, and a derived page action while asserting one submission and the specified visible result/navigation.
...
- [x] 7.8 Add `bloc_test` and Tier-2 Share component coverage with real Cubit/use cases for scanned/imported images, absent extracted-text action, export success/cancel/failure, platform differences, progress, long names, dark mode, tablet, and offline use.
- [x] 7.9 Add fixture-backed `@Preview()` entries and phone/tablet light/dark/large-text goldens for Share default/loading/empty/error/long-content, preparation, handoff, export completion, cancellation, and each failure stage.
- [x] 7.10 Update the `share` robot and Android/iOS Tier-3 flow to share imported PDF pages as images, prove extracted text is not offered, export successfully, cancel without error, and recover from deterministic provider failure.
...
- [x] 8.4 Verify share/export, PDF rendering, OCR, and navigation through the Android Tier-3 gate and host coverage. The iOS Simulator is locally blocked because the ML Kit dependencies lack arm64 simulator support; physical iOS testing is excluded by project policy.
...
- [x] 9.10 Re-run formatting, analysis, layering/platform checks, Tier-1/Tier-2/goldens/coverage, Android Tier-3 flows, and `tool/verify.dart`; resolve every regression before archive. iOS Simulator execution is recorded as unavailable because the ML Kit dependencies lack arm64 simulator support, and physical iOS is excluded by project policy.
