...
Share-text availability is derived from actual persisted or extractable content rather than trusting only `Document.hasRecognisedText`. Loading the share sheet checks the text store and inexpensive PDF capability metadata; choosing Share Text performs extraction if needed and reports progress. After successful extraction, the document summary flag is repaired as a derived cache. Passwords are resolved through the existing secure-storage boundary and never enter Cubit state, logs, fixtures, or serialized models.
...
Replace the document/folder list presentation with one sliver-based browsing surface. Compact widths render exactly two columns. Wider widths derive the column count from available content width and a minimum readable tile extent, allowing iPad multitasking and Android tablets to settle naturally at three or more columns without device-name checks. Grid spacing, horizontal margins, and tile width come from a small documented layout policy so previews and goldens exercise deterministic breakpoints.
...
The shared sheet has `pdf_edit_operation_sheet`, `pdf_edit_review`, `pdf_edit_confirm`, `pdf_edit_cancel`, and `pdf_edit_result`. Existing operation keys remain entry points. Split requires a boundary and two collision-safe output names, then shows both outputs; each can be opened and Done returns to Dashboard. Merge requires document selection and ordering, then opens the one created result. Compress explicitly states that it replaces the current PDF and reports before/after size while staying in Editor. Watermark and Protect review all inputs and update the current document in place. Page extract/duplicate and other derived actions show the same progress/result contract and open exactly one result where applicable.
...
Share-images obtains handles from page access, renders only selected pages with bounded concurrency, retains temporary files through the share handoff, then cleans them. Share-text invokes the unified extraction use case. Share state distinguishes preparing content, handing off to the platform, exporting, completed, cancelled, and failed stages so messages identify what the user can retry.
