...
- Add a persistent Large/Small Dashboard display control. Large preserves the current two-column phone layout; Small uses a denser three-column phone layout and proportionally more iPad/tablet columns so previews occupy roughly 40–50% of the current area without losing two-line names or accessible targets.
...
- Make camera capture explicitly user-driven: show the live camera before capture, never fire the shutter automatically, return from Enhance to Crop, preview flip immediately, enlarge crop-handle hit regions, smooth drag updates, and use compact confirmation titles. Replace the multi-page naming popup with a dedicated scrollable page-naming screen whose header provides Cancel and Done.
- Put Print, Compress, Split, Watermark, and Set/Remove Password directly in the Viewer overflow menu. Each operation opens its focused sheet or screen without first presenting a generic editor hub.
...
- Extract embedded text from text-based PDFs using the existing PDF engine before falling back to on-device OCR for image-only pages. Keep that text available to internal search and recognition features, but remove the unreliable Share Extracted Text action from the share surface.
...
- Make the Settings PDF-quality chooser scroll-safe and safe-area-aware, and add bottom spacing below Privacy Policy so the last row is visually separated from the tab bar/home indicator.
...
- `document-viewer`: Make long titles and actions responsive, replace the ambiguous persistent page-jump field with a compact explicit page-navigation flow, and expose focused PDF operations directly from the overflow menu.
- `pdf-editing`: Require focused operation sheets/screens, confirmation, progress, duplicate-submission protection, success outcomes, and purposeful navigation for split, merge, compress, watermark, protect, and other derived/in-place operations.
- `document-import`: Require imported PDFs to expose stable page identities and lazily renderable page content without permanently duplicating all page images; make camera/crop/enhance and multi-page naming predictable and user-driven.
- `ocr`: Support embedded-text extraction and OCR fallback for imported PDFs, with truthful persisted text availability.
- `document-sharing`: Make image sharing and device export work for imported PDFs, remove Share Extracted Text from the user-facing sheet, use platform-correct export behavior, and present actionable stage-specific failures and completion feedback.
- `app-settings`: Make the PDF-quality chooser scroll-safe and preserve comfortable bottom spacing around the final Privacy Policy row.
...
lib/
  ...
  features/
    ...
    document_sharing/
      application/usecases/          # page-source-aware image/export flows
...
- Tier 3 flows cover Dashboard density/search and adaptive grid browsing, multi-select archive/trash without tile reflow, manual camera capture with reversible Crop/Enhance, dedicated page naming, moving Detail into a newly created folder, reviewed duplicate naming/destination, direct one-shot PDF operations, imported PDF previews, internal embedded-text indexing, image sharing, export success/cancellation, and actionable failures exclusively through keys and semantics.
