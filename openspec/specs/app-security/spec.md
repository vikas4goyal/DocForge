...
Define the application's privacy and security posture: private working data stays device-local, finished PDFs remain user-controlled and visible in the selected local or iCloud library, and PDF passwords are per-document, user-chosen, confirmed at creation and held only in secure storage.
