...
Define the first-launch experience: a one-time flow that welcomes the user, states the local-only and offline guarantees, asks for camera permission with a rationale, and then hands the user to the dashboard — never shown again once completed.

## Requirements

### Requirement: First-launch onboarding flow
The application SHALL present the onboarding flow exactly once, on the first launch after installation, in the order Welcome → Privacy & Offline Introduction → Camera Permission Request → dashboard.
...
- **AND** the dashboard is not reachable until onboarding completes
...
- **THEN** the onboarding-completed flag is written to preferences before navigation to the dashboard occurs

### Requirement: Returning users skip onboarding
The application SHALL navigate directly to the dashboard on every launch after onboarding has been completed.
...
The application SHALL request camera permission during onboarding with a rationale shown before the system dialog, and SHALL allow the user to proceed to the dashboard whether or not permission is granted.
...
- **AND** the camera source in the creation flow is enabled

#### Scenario: Permission denied
- **WHEN** the user denies camera permission
- **THEN** the user is still navigated to the dashboard
- **AND** the camera source remains visible and, when activated, presents the permission-recovery path
...
- **THEN** the full onboarding flow completes and the user reaches the dashboard without any error or network request
