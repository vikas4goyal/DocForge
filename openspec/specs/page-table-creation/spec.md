# page-table-creation Specification

## Purpose

Define PDF creation as a single screen: an ordered table whose rows are the pages of the document being built. Covers adding pages from the camera or the photo library through crop and enhancement, reordering, per-row crop/enhance/delete, naming and saving with optional password protection, abandoning a session, and the accessibility, theming, layout and offline baselines the screen must meet.
