...
///
/// Proves that [buildDocForge] assembles the same tree `main.dart` used to
...
void main() {
  ...
  setUp(() async {
    cacheDirectory = await Directory.systemTemp.createTemp('docforge_cache_');
    documentsDirectory = await Directory.systemTemp.createTemp('docforge_doc_');
    databaseDirectory = await Directory.systemTemp.createTemp('docforge_db_');
    derivedDirectory = await Directory.systemTemp.createTemp('docforge_der_');
    // A distinct instance per test, because the previous one may still be
    // closing: a screen can leave a query in flight that only completes outside
    // the fake-async zone, and Isar will not reuse a name until it has.
    bootCount++;
    isar = await Isar.open(
      ...
      name: 'doc_forge_test_$bootCount',
    );
  });

  tearDown(() async {
    // Not awaited: a screen the test pumped can leave a library query in
    // flight, and `close` waits for it. The query only ever completes outside
    // the fake-async zone the test body ran in, so awaiting here would hang
    // rather than clean up. Each test opens its own instance and directory, so
    // an instance that closes a moment late affects nothing.
    ...
    for (final directory in [
      ...
      // The database directory is left to the operating system's temp sweep:
      // deleting it while the instance is still closing would race.
      if (directory != databaseDirectory && directory.existsSync()) {
  ...
  Future<Widget> boot(
    ...
    () => buildDocForge(
      ...
      publicStore: publicStore ?? FilesystemPublicFileStore(documentsDirectory),
  ...
  group('buildDocForge', () {
    testWidgets('builds the same tree main.dart used to build inline', (
      tester,
    ) async {
      ...
      final app = find.byType(DocForgeApp);
    ...
    testWidgets('hands the supplied dependencies to the root widget', (
      ...
      final app = tester.widget<DocForgeApp>(find.byType(DocForgeApp));
      expect(app.dependencies, same(dependencies));
    });

    testWidgets('initialises the supplied public store rather than its own', (
      tester,
    ) async {
    ...
    testWidgets('lands on onboarding when the flag has never been written', (
      tester,
    ) async {
      await tester.pumpWidget(await boot(tester));
      await settle(tester);
    ...
    testWidgets('lands on the dashboard once onboarding is complete', (
      tester,
    ) async {
      final dependencies = buildFakeAppDependencies();
      await dependencies.preferences.writeBool(
    ...
    testWidgets('honours the supplied authenticator on the unlock screen', (
      tester,
    ) async {
      final dependencies = buildFakeAppDependencies();
      await dependencies.preferences.writeBool(
    ...
    testWidgets('starts at the requested initial location', (tester) async {
      final dependencies = buildFakeAppDependencies();
      await dependencies.preferences.writeBool(
        PreferenceKeys.onboardingComplete,
        true,
