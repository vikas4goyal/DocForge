...
Future<void> _resolveNative({
  ...
  // Upstream publishes only STATIC `.a` binaries for iOS, but Flutter
  // requests `dynamic` and rejects a static asset outright:
  //
  //   CodeAsset ... has a link mode "static", which is not allowed by
  //   the input link mode preference "dynamic"
  //
  // With no `ios-*-libpdf_oxide.dylib` release artifact to download, the
  // stock hook falls through to compiling the Rust engine from source,
  // which needs a Rust toolchain on every machine that builds iOS — CI
  // included. See third_party/pdf_manipulator/LOCAL_PATCH.md.
  //
  // The `.a` already contains the compiled machine code, so we resolve it
  // through upstream's normal hash-verified waterfall and relink it into a
  // dylib with clang. Xcode is already a prerequisite for building iOS, so
  // this adds no new tooling. Delete this block once upstream ships the
  // dylib and the pubspec points back at the pub.dev release.
  if (!custom && buildInput.config.code.targetOS == OS.iOS) {
    await _resolveIosDylibFromStatic(
      constants: constants,
...
// ── LOCAL PATCH (not upstream) ───────────────────────────────────
// Produces the iOS dylib Flutter demands from the static archive upstream
// actually publishes. See the call site in _resolveNative for why.

/// Resolves the published static `.a` for [targetKey] through the normal
/// hash-verified waterfall, then relinks it into a dylib at [dest].
///
/// The archive is fetched into [cacheDir] and kept there, so repeat builds
/// re-link from cache instead of re-downloading ~160MB.
Future<void> _resolveIosDylibFromStatic({
  required BuildConstants constants,
  required Uri packageRoot,
  required String version,
  required String targetKey,
  ...
  const staticLib = 'libpdf_oxide.a';
  final staticAsset = '$targetKey-$staticLib';
  final archive = File(p.join(cacheDir.path, staticLib));

  await resolveAsset(
    ResolveRequest(
      assetName: staticAsset,
      downloadUrl: constants.downloadUrl(version, staticAsset),
      dest: archive,
      cacheFile: archive,
      expectedHash: assetHashes[staticAsset],
      version: version,
      packageRoot: packageRoot,
      // No source fallback: this lane exists precisely to avoid needing a
      // Rust toolchain. If the pinned archive can't be had, say so plainly
      // rather than silently starting a from-source compile.
      compile: (File d) => throw StateError(
        'Could not obtain $staticAsset, and this build is configured to '
        'relink the published static archive rather than compile from '
        'source. Check network access to GitHub Releases.',
      ),
    ),
  );

  await _relinkStaticToDylib(archive: archive, dest: dest, code: code);
}

/// Relinks [archive] into a dynamic library at [dest] using clang from the
/// active Xcode toolchain.
Future<void> _relinkStaticToDylib({
  ...
  _log.info('relinking ${archive.path} → ${dest.path} ($arch, $sdk)');

  // -all_load keeps every object from the archive: the linker would
  // otherwise drop objects nothing references, and the FFI entry points
  // have no references *inside* the library — they are looked up by name
  // from Dart at runtime.
  final result = await Process.run('xcrun', [
    ...
    arch,
    '-isysroot',
    (sdkPath.stdout as String).trim(),
    '-target',
    '$arch-apple-ios$minVersion${simulator ? '-simulator' : ''}',
    '-Wl,-all_load',
    archive.path,
    '-install_name',
    '@rpath/libpdf_oxide.dylib',
    '-framework',
    'Foundation',
    '-framework',
    'CoreFoundation',
  ...
  if (result.exitCode != 0) {
    throw StateError(
      'Relinking ${archive.path} into a dylib failed '
